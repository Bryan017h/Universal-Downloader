...
class DownloaderMixin:
    def run_download_manager(self, url, folder, mode, res, audio_fmt, use_hb, hb_preset, trim_on, t_start, t_end):
        ...
        # 2. If it failed (status == 1) and wasn't stopped by user, report error immediately.
        if status == 1 and not self.stop_event.is_set():
            self.finish_fail("Download failed. Please check the URL or your internet connection.")
